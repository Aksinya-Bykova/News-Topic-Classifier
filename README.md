...
# from 751 to 12 ms!!! (see last picture)
...
-----
Я сделала весь этот проект только ради того, чтобы поиграться с оптимизацией. Узнала, что такое onnx - такая библиотека на C++ для ускорения выполнения операций из торча. Напоминает -O3 компиляцию в C++, когда компилятор подбирает, как физически выполнится программа. Тут есть свой аналог дизассемблера:
